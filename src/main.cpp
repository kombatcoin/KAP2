...
uint256 hashGenesisBlock("0x00000135fda1484ef7fbef73885b30550673e2bed0624d05272ef4d74f64fc96"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // KappaCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "KappaCoin Signed Message:\n";
...
{
    // KappaCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // KappaCoin: 1 day
static const int64 nTargetSpacing = 2.5 * 60; // KappaCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // KappaCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, kappacoin - DarkGravity v2, written by Evan Duffield - evan@kappacoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static DarkGravityWave3(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, kappacoin - DarkGravity v3, written by Evan Duffield - evan@kappacoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckVotes) const
{
    ...
    // KappaCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // KappaCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        hashGenesisBlock = uint256("0x00000135fda1484ef7fbef73885b30550673e2bed0624d05272ef4d74f64fc96");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "KappaCoin - True CPM is Here! 7.29.2014";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1406678253;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1034638;
        ...
        assert(block.hashMerkleRoot == uint256("0x8e263de604eac4c11c5be5e1042a0e554d29dc95364ab154f78e3edebe93ba01"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("KappaCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("KappaCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static KappaCoinMiner(CWallet *pwallet)
{
    printf("KappaCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("kappacoin-miner");
    ...
    try { loop {
        ...
        printf("Running KappaCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("KappaCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&KappaCoinMiner, pwallet));
}
...
/* *** BEGIN DARKSEND MAGIC - KAPPACOIN **********
    Copyright 2014, KappaCoin Developers 
        eduffield - evan@kappacoin.io
        InternetApe - kyle@kappacoin.io
*/
